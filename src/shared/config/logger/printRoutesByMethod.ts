...
export function printRoutesByMethod(app: NestFastifyApplication) {
  const fastify = app.getHttpAdapter().getInstance();
  const allRoutes = fastify.printRoutes({ commonPrefix: false, includeMeta: true });

  const methodMap: { [key in 'GET' | 'POST' | 'PUT' | 'PATCH' | 'DELETE']: string[] } = {
    GET: [],
    POST: [],
    PUT: [],
    PATCH: [],
  ...
  for (const line of lines) {
    ...
    const match = line.match(/[\s│├└─]*([\w\/\-\_\*]+) \((GET|POST|PUT|PATCH|DELETE|HEAD|OPTIONS)\)/);
